...
public class AdminProductPageController {
    ...
    public String list(ProductSearchRequestDto requestDto, Model model, @AuthenticationPrincipal MyUserDetails myUserDetails) {
        ...
        if(member == null) {
            ...
            PageResponseDTO<ProductSummaryResponseDTO> products = productService.searchProductsBySeller(requestDto, member);
            log.info("프로덕트 " + products);
            model.addAttribute("products", products);
            model.addAttribute("searchDto", requestDto);
        }
